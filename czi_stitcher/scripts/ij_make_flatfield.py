...
#
# ImageJ --ij2 --headless --run imagej_basic_ashlar.py "input='input.ext',
#   output_dir='output',output_name='my_experiment'"

import sys
from ij import IJ, WindowManager
from ij.macro import Interpreter
from loci.plugins import BF
from loci.plugins.in import ImporterOptions  # noqa: E225 E999
...
def main():
    ...
    # reading every single image. The series count (num_images) is the one
    # value we can't easily get any other way, but we might as well grab the
    # others while we have the reader available.
    ...
    for channel in range(num_channels):
        ...
        ff_channel = WindowManager.getImage("Flat-field:%s" %
                                            input_image.title)
        ff_image.slice = channel + 1
        ff_image.getProcessor().insert(ff_channel.getProcessor(), 0, 0)
        ff_channel.close()
        df_channel = WindowManager.getImage("Dark-field:%s" %
                                            input_image.title)
